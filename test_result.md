...
agent_communication:
    ...
    - agent: "main"
      message: "v2.5.0 — UI compattazione richiesta utente: (1) Home: pulsanti Celebra/Orazionale ridotti da minHeight 160→96 con icona inline accanto al testo, niente più subtitle. (2) Impostazioni: rimossa completamente la sezione 'Modalità lettura' — tap-to-advance è ora il default fisso (readingMode resta nel context per retrocompatibilità ma non più esposto). (3) Top bar messa: paddingVertical 14→4, backBtn minHeight 64→44, content padding 20→16/8/32. partBox marginBottom 28→12, tutti i titoli colorati (sectionTitle/ritoTitle/antifona/reading/orazione/subtitle): marginTop 14→6, marginBottom 8→4. text marginVertical 8→4, lineHeight 1.55→1.45. readingBlock marginVertical 14→6. Risultato: testo riempie il 90% in più dello schermo. (4) PE: nuovo stile selectorBtnInline + titleRow per affiancare bottone 'Scegli' al titolo 'Preghiera Eucaristica' (era una riga completa sotto). peSelectorBtn ridotti drasticamente: paddingVertical 14→6, minHeight 76→40, sfondo trasparente con bordo ambra sottile (era sfondo ambra pieno con bordo 3px). HEADER_FOOTER_OVERHEAD ridotto a 130 e TEXT_HORIZONTAL_PADDING a 32 per riflettere il nuovo layout. Verificato via screenshot 1280x800: PE Consacrazione mostra titolo+Scegli+selettori discreti in 1 riga ciascuno, poi tutto il testo PE II fino a 'prese il pane'."
